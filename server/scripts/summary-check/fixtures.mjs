...
export async function createFixture(mode) {
  ...
  const board = await Board.create({ name: 'Summary verification', owner: user._id, members: [{ user: user._id, role: 'owner' }] });
  const card = mode === 'commits-only' ? null : await Card.create({ board: board._id, list: new mongoose.Types.ObjectId(), title: 'API resilience', status: 'Done', position: 1 });
  if (mode !== 'unlinked') {
    ...
    await BoardGitHubIntegration.create({ board: board._id, connectedBy: user._id, githubAccount: account._id,
      repoId: '1', repoOwner: 'example', repoName: 'project', repoFullName: 'example/project', repoUrl: 'https://github.com/example/project', defaultBranch: 'main' });
...
export function providerFetch(getFixture) {
  return async (url, options) => {
    ...
    if (url.startsWith('https://api.github.com/repos/')) {
      ...
      return new Response(JSON.stringify([{ sha, commit: { message: 'Improve API resilience\nPRIVATE_COMMIT_BODY', author: { name: 'PRIVATE_AUTHOR', date: '2026-09-11T00:00:00Z' } } }]));
    }
    if (url === 'https://api.openai.com/v1/responses') {
      ...
      const output = Object.fromEntries(tasks.map((group) => [group.section, group.cards.length ? [{ text: 'Task snapshot verified.', cardIds: [group.cards[0].id] }] : []]));
