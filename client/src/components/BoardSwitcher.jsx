...
// A dropdown in the project header that lists all of the user's projects and
// lets them jump straight to another one with no trip back to the dashboard.
...
export default function BoardSwitcher({ currentBoard }) {
  ...
  // Fetch the project list the first time the menu is opened, then cache it.
  useEffect(() => {
    ...
    async function loadBoards() {
      try {
        ...
        // Non-fatal: the switcher just stays empty. The project itself still loads.
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      >
        ...
        <span className="truncate">{currentBoard?.name || 'Project'}</span>
        ...
        >
          ...
          <div className="max-h-72 overflow-y-auto py-1">
            {!loaded ? (
              ...
              <p className="px-3 py-2 text-sm text-zinc-500 dark:text-zinc-400">No other projects.</p>
            ) : (
              boards.map((b) => {
