...
// Project creation is intentionally identity-only. Workflows are added inside
// the project so the mental model stays: project container -> workflows -> cards.
export default function NewBoardModal({
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      toast.error(editing ? 'Could not update project' : 'Could not create project', err.message)
      setSubmitting(false)
    }
  }
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-label={editing ? 'Edit project' : 'Create a project'}
      ...
      >
        <form onSubmit={handleSubmit}>
          <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-4 dark:border-zinc-800">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                {editing ? 'Update this project identity.' : 'Name the project first; add workflows once you are inside.'}
              </p>
            </div>
            <button
              type="button"
          ...
          <div className="grid gap-5 px-5 py-5 md:grid-cols-[minmax(0,1fr)_220px]">
            ...
            <aside className="rounded-lg border border-dashed border-zinc-200 bg-zinc-50 p-3 dark:border-zinc-800 dark:bg-zinc-950">
              ...
              {!editing && (
                <div className="mt-4 rounded-lg border border-zinc-200 bg-white p-3 text-xs leading-5 text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400">
                  New projects start with a default General workflow. Add Sprint, Bug Triage, Release Plan, or a custom workflow from inside the project.
                </div>
              )}
            </aside>

            {error && (
              <p className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600 dark:bg-red-500/10 dark:text-red-300 md:col-span-2">
                {error}
              </p>
            )}
          </div>
