...
function dueDateMeta(dueDate) {
  ...
  if (daysAway < 0) {
    return { label: formatter.format(date), tone: 'border-red-200 bg-red-50 text-red-700 dark:border-red-500/30 dark:bg-red-500/10 dark:text-red-300' }
  }
  if (daysAway <= 2) {
    return { label: formatter.format(date), tone: 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200' }
  }

  return { label: formatter.format(date), tone: 'border-zinc-200 bg-zinc-50 text-zinc-500 dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-400' }
}
...
export default function SortableCard({ card, onOpen }) {
  ...
  const due = dueDateMeta(card.dueDate)
    ...
    >
      ...
      {(assignedTo || due) && (
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          {due && (
            <span className={`inline-flex shrink-0 items-center gap-1 rounded-md border px-1.5 py-1 text-[11px] font-semibold ${due.tone}`}>
